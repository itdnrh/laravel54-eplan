...
class PlanMaterialController extends Controller
{
    ...
    public function store(Request $req)
    {
        ...
        if($plan->save()) {
            ...
            $material->price_per_unit   = currencyToNumber($req['price_per_unit']);
            $material->unit_id          = $req['unit_id'];
            $material->amount           = currencyToNumber($req['amount']);
            $material->sum_price        = currencyToNumber($req['sum_price']);
            $material->remain_amount    = currencyToNumber($req['amount']);
            $material->remain_budget    = currencyToNumber($req['sum_price']);
            // $material->request_cause   = $req['request_cause'];
            $material->have_amount      = currencyToNumber($req['have_amount']);
