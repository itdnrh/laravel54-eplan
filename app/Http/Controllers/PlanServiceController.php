...
class PlanServiceController extends Controller
{
    ...
    public function store(Request $req)
    {
        ...
        if($plan->save()) {
            ...
            $service->price_per_unit    = currencyToNumber($req['price_per_unit']);
            $service->unit_id           = $req['unit_id'];
            $service->amount            = currencyToNumber($req['amount']);
            $service->sum_price         = currencyToNumber($req['sum_price']);
            $service->remain_amount     = currencyToNumber($req['amount']);
            $service->remain_budget     = currencyToNumber($req['sum_price']);
            // $service->request_cause     = $req['request_cause'];
            // $service->have_amount       = currencyToNumber($req['have_amount']);
