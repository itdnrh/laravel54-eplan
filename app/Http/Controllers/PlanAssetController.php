...
class PlanAssetController extends Controller
{
    public function formValidate (Request $request)
    {
        ...
        $messages = [
            ...
            'item_id.required'          => 'กรุณาเลือกรายการครุภัณฑ์',
    ...
    public function store(Request $req)
    {
        ...
        if($plan->save()) {
            ...
            $asset->spec            = $req['spec'];
            $asset->price_per_unit  = currencyToNumber($req['price_per_unit']);
            $asset->unit_id         = $req['unit_id'];
            $asset->amount          = currencyToNumber($req['amount']);
            $asset->sum_price       = currencyToNumber($req['sum_price']);
            $asset->remain_amount   = currencyToNumber($req['amount']);
            $asset->remain_budget   = currencyToNumber($req['sum_price']);
            $asset->request_cause   = $req['request_cause'];
            $asset->have_amount     = currencyToNumber($req['have_amount']);
